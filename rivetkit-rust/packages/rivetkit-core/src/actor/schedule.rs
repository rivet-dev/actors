...
use crate::actor::internal_storage::{
	optional_owned_text_param, read_trace_origin, trace_origin_params,
};
...
impl ActorContext {
	...
	pub async fn at(&self, timestamp_ms: i64, action_name: &str, args: &[u8]) -> Result<String> {
		...
		schedule_params.extend(trace_origin_params(origin));
		self.sql()
	...
	async fn upsert_recurring(
		...
		params.extend(trace_origin_params(origin));
		self.sql()
...
fn read_due_schedule(row: &[ColumnValue]) -> Result<(StoredSchedule, TraceOrigin)> {
	...
	let origin = read_trace_origin(row, 10, "schedule trace origin")?;
	Ok((event, origin))
}
