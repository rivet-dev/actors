...
mod moved_tests {
	...
	#[tokio::test]
	async fn state_transaction_rejects_multi_statement_exec() {
		let ctx = new_with_kv(
			"actor-state-tx-exec",
			"state-tx-exec",
			Vec::new(),
		...
		let error = transaction
			...
			.await
			.expect_err("state-aware transactions must reject exec");
		assert!(format!("{error:#}").contains("single-statement execute"));
		transaction
			.rollback()
			.await
			.expect("rejected exec should leave the transaction rollback-safe");
	}
