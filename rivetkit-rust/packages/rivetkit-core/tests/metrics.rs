...
mod moved_tests {
	...
	fn sqlite_profiling_metrics_render_statement_and_transaction_end_to_end() {
		...
		assert_metric_value_with_labels(
			&rendered,
			"rivetkit_sqlite_duration_seconds_count",
			actor_name,
			&[
				"type=\"statement\"",
		...
		for phase in ["transaction_wait", "worker_wait", "storage", "local_work"] {
			assert_metric_value_with_labels(
				&rendered,
				"rivetkit_sqlite_phase_duration_seconds_count",
				actor_name,
		...
		assert_metric_value_with_labels(
			&rendered,
			"rivetkit_sqlite_transaction_statement_count_count",
			actor_name,
		...
		assert_metric_value_with_labels(
			&rendered,
			"rivetkit_sqlite_get_pages_duration_seconds_count",
			actor_name,
	...
	fn sqlite_fast_statement_requires_repetition_before_fingerprint_admission() {
		...
		assert!(rendered.lines().any(|line| {
			line.starts_with("rivetkit_sqlite_duration_seconds_count{")
	...
	fn sqlite_disabled_profiling_records_no_profile_metrics_or_events() {
		...
		assert!(!rendered.lines().any(|line| {
			line.contains(&format!("actor_name=\"{actor_name}\""))
				&& line.starts_with("rivetkit_sqlite_")
		}));
	}
	...
	async fn sqlite_operations_report_profiles_through_the_native_stack() {
		...
		assert!(
			...
			"repeated statements should render under one select fingerprint:\n{rendered}"
		);
		assert!(
			rendered.lines().any(|line| {
				line.starts_with("rivetkit_sqlite_outcome_total{")
					&& line.contains(&format!("actor_name=\"{actor_name}\""))
		...
		for byte_kind in [
			...
			assert!(
				rendered.lines().any(|line| {
					line.starts_with("rivetkit_sqlite_local_bytes_total{")
						&& line.contains(&format!("actor_name=\"{actor_name}\""))
