...
Rivet Cloud manages streaming request routing automatically. On self-hosted deployments, set `features.guard_gateway_v3.mode` to `opportunistic` or `on` and set `features.guard_gateway_v3.percentage` to `100` to guarantee coverage for workers that support protocol v7. Lower percentages deterministically sample actors, older workers stay on the buffered request path, and the default mode is `off`.
