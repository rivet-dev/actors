...
- If you need more diagnostics, set `RIVET_LOG_LEVEL=DEBUG` for verbose logging. See [Logging](/actors/docs/general/logging) for more options.
...
See [Actor Statuses](/actors/docs/statuses) for more about this status.
...
No server was available to run your actor. The cause depends on your [runtime mode](/actors/docs/general/runtime-modes):
...
- There is an issue connecting to your backend. If the control plane is hitting `/api/rivet/start` and failing, check your backend logs for errors.
...
No server was available to run your actor. This is equivalent to `no_capacity` on the current allocation path. See the `no_capacity` section above for the causes and fixes for your [runtime mode](/actors/docs/general/runtime-modes).
...
If your actors are still running old code after deploying a new version, your [versioning](/actors/docs/versions) is likely not configured correctly.

Without versioning, Rivet has no way to distinguish old deployments from new ones. The behavior depends on your [runtime mode](/actors/docs/general/runtime-modes):
...
To fix this, configure a version number in your [registry configuration](/actors/docs/general/registry-configuration). When a new version is deployed, Rivet will allocate new actors to the latest version and optionally drain old actors to migrate them.

## Actor status is pending

See [Actor Statuses](/actors/docs/statuses) for more about this status.

An actor stays in "pending" status when Rivet is waiting for a server to accept it. The cause depends on your [runtime mode](/actors/docs/general/runtime-modes):
