...
[Durable Streams](https://github.com/durable-streams/durable-streams) is a protocol for append-only streams with catch-up reads, live updates, and resumable offsets. Rivet provides a managed implementation backed by Rivet Actors.
...
The command prints the local URL when it is ready. Durable Streams is available at `http://127.0.0.1:8787/durable-streams/v1/stream/<path>`.
...
curl -i -X PUT \
  ...
  --data '[{"message":"hello"}]' \
  http://127.0.0.1:8787/durable-streams/v1/stream/demo
...
curl 'http://127.0.0.1:8787/durable-streams/v1/stream/demo?offset=-1'
```
