...
describe("RivetError bridge helpers", () => {
	...
	test("decodes bridge payloads whose ray ID serialized to null", () => {
		// The native bridge emits `"rayId": null` when no ray ID exists; the
		// payload must still decode as a structured error rather than degrade.
		const encoded = `${BRIDGE_RIVET_ERROR_PREFIX}${JSON.stringify({
			group: "actor",
			code: "boom",
			message: "typed failure",
			metadata: { source: "native" },
		...
		const decoded = decodeBridgeRivetError(encoded);
		...
		expect(decoded?.rayId).toBeUndefined();
	});
